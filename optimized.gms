...
Scalar W "minimum up/down time (time steps)";
W = 48;
Scalar Woverlap;
Woverlap = max(0, W-1);
...
Scalar tHistStart;
Parameter
    wStart(Window)      "decision start index per window"
    wStartHist(Window)  "history start (overlap) per window"
    wEnd(Window)        "end index per window";
wStart(Window)     = 1 + (ord(Window)-1)*ShiftLen;
wStartHist(Window) = wStart(Window) - Woverlap;
wStartHist(Window)$(wStartHist(Window) < 1) = 1;
wEnd(Window)       = (ord(Window)-1)*ShiftLen + HorizonLen;
Set Twindow(Window,T) "precomputed active time steps per window";
Twindow(Window,T)$((ord(T) >= wStartHist(Window)) and (ord(T) <= wEnd(Window))) = yes;
...
Parameter
    ...
    carry_state(J,N)  "pump status at window end"
    carry_run(J,N)    "consecutive time steps in current status"
    lockLen(J,N)      "remaining min up/down lock at window start"
    lastSwitchOrd(J,N)"last switch time index in window"
    cbar0(J)          "initial average concentration per segment";

* Initial carry-over values (t0)
carry_M(J)      = 40000;
carry_K(J,N)    = 0;
carry_state(J,N) = carry_K(J,N);
carry_run(J,N)   = W;
...
Equations
    ...
    NoSimulSwitch(T,J,N),
    SwitchPosAtT0(J,N),    SwitchNegAtT0(J,N),    KtermHold(T,J,N);
SwitchPosDetect(T,J,N)$(P(J,N) and TactVar(T) and ord(T) > 1)..    SwitchPos(T,J,N) =g= K(T,J,N) - K(T-1,J,N);
SwitchNegDetect(T,J,N)$(P(J,N) and TactVar(T) and ord(T) > 1)..    SwitchNeg(T,J,N) =g= K(T-1,J,N) - K(T,J,N);
*SwitchPosDetect(T,J,N)$(P(J,N) and TactEq(T) and ord(T) > tHistStart)..
*    SwitchPos(T,J,N) =g= K(T,J,N) - K(T-1,J,N);

*SwitchNegDetect(T,J,N)$(P(J,N) and TactEq(T) and ord(T) > tHistStart)..
*    SwitchNeg(T,J,N) =g= K(T-1,J,N) - K(T,J,N);
SwitchPosUB(T,J,N)$(P(J,N) and TactVar(T) and ord(T) > 1)..    SwitchPos(T,J,N) =l= K(T,J,N);
SwitchNegUB(T,J,N)$(P(J,N) and TactVar(T) and ord(T) > 1)..    SwitchNeg(T,J,N) =l= 1 - K(T,J,N);
NoSimulSwitch(T,J,N)$(P(J,N) and TactVar(T) and ord(T) > 1)..   SwitchPos(T,J,N) + SwitchNeg(T,J,N) =l= 1;
...
Equation KtermHold(T,J,N);
KtermHold(T,J,N)$(P(J,N) and Tterm(T) and ord(T) > 1)..    K(T,J,N) =e= K(T-1,J,N);

*$ontext
Equations     MinUpTime(T,J,N),    MinDownTime(T,J,N),    MinUp_Prefix(T,J,N),    MinDown_Prefix(T,J,N);
MinUpTime(T,J,N)$(P(J,N) and TactVar(T) and ord(T) >= tStartIdx + W)..    sum(Tp$((TactVar(Tp)) and (ord(Tp) > ord(T)-W) and (ord(Tp) <= ord(T))),    SwitchPos(Tp,J,N)) =l= K(T,J,N);
MinDownTime(T,J,N)$(P(J,N) and TactVar(T) and ord(T) >= tStartIdx + W)..  sum(Tp$((TactVar(Tp)) and (ord(Tp) > ord(T)-W) and (ord(Tp) <= ord(T))),    SwitchNeg(Tp,J,N)) =l= 1 - K(T,J,N);
MinUp_Prefix(T,J,N)$(P(J,N) and TactVar(T) and ord(T) < tStartIdx + W)..  sum(Tp$((TactVar(Tp)) and (ord(Tp) <= ord(T))), SwitchPos(Tp,J,N)) =l= K(T,J,N);
MinDown_Prefix(T,J,N)$(P(J,N) and TactVar(T) and ord(T) < tStartIdx + W)..sum(Tp$((TactVar(Tp)) and (ord(Tp) <= ord(T))), SwitchNeg(Tp,J,N)) =l= 1 - K(T,J,N);
*$offtext
...
loop(Window,
    tHistStart = smin(T$Twindow(Window,T), ord(T));
    tStartIdx  = wStart(Window);
    tEndIdx    = wEnd(Window);
    ...
    TactEq(T)  = no;
    TactEq(T)$(Twindow(Window,T) and ord(T) >= tStartIdx and ord(T) < tEndIdx) = yes;

    Tinit(T)   = no;
    Tterm(T)   = no;
    Tinit(T)$(ord(T) = tHistStart) = yes;
...
*    * enforce min up/down across window boundaries
    lockLen(J,N) = max(0, W - carry_run(J,N));
    K.fx(T,J,N)$(TactEq(T) and ord(T) >= tStartIdx and ord(T) < tStartIdx + lockLen(J,N)) = carry_state(J,N);

*    * allow decisions inside window (exclude history overlap)
    K.lo(T,J,N)$TactEq(T) = 0;
    K.up(T,J,N)$TactEq(T) = 1;
...
*    * store terminal states for next window
    ...
    carry_state(J,N) = carry_K(J,N);
    lastSwitchOrd(J,N) = smax(T$(TactVar(T) and (SwitchPos.l(T,J,N) + SwitchNeg.l(T,J,N) > 0.5)), ord(T));
    lastSwitchOrd(J,N)$(lastSwitchOrd(J,N) < 1) = 0;
    carry_run(J,N)$(lastSwitchOrd(J,N) = 0) = min(W, carry_run(J,N) + (tEndIdx - tStartIdx + 1));
    carry_run(J,N)$(lastSwitchOrd(J,N) > 0) = min(W, tEndIdx - lastSwitchOrd(J,N) + 1);
